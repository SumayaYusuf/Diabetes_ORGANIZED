...
# Define a neural network with a configurable activation function
class Predictor(nn.Module):
    def __init__(self, input_dim, activation_func):
...
# Ablation study with different activation functions
...
for act_name, act_func in activation_functions.items():
    experiment_data[act_name] = {}
    for lr in learning_rates:
        model = Predictor(input_dim=4, activation_func=act_func).to(device)
        ...
        for epoch in range(n_epochs):
            ...
            print(
                f"Activation {act_name}, LR {lr}, Epoch {epoch+1}: Train Loss = {train_loss:.4f}, Val Loss = {val_loss:.4f}"
            )
        ...
        print(f"Activation {act_name}, Learning Rate {lr}: ATT = {ATT:.4f}")

        # Save the results for the current experiment
        experiment_data[act_name][f"lr_{lr}"] = results
