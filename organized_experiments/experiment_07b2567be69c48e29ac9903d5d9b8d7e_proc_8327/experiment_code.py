...
for opt_name, opt_class in optimizers.items():
    ...
    for epoch in range(n_epochs):
        ...
        print(
            f"Optimizer {opt_name}, Epoch {epoch+1}: Train Loss = {train_loss:.4f}, Val Loss = {val_loss:.4f}"
        )
    ...
    print(
        f"Optimizer {opt_name}: Average Treatment Effect on the Treated (ATT): {ATT:.4f}"
    )

    # Save results for each optimizer
    experiment_data[f"optimizer_{opt_name}"] = results
