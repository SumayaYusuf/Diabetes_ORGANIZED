...
# Convert to tensors without 'Cholesterol'
def prepare_tensor_data(df):
    X = df[["Age", "Exercise", "Medication"]].values
    y = df["HbA1c"].values
...
for lr in learning_rates:
    ...
    treated_effect = model(
        torch.tensor(
            treated_group[["Age", "Exercise", "Medication"]].values,
            dtype=torch.float32,
        ).to(device)
    )
    control_effect = model(
        torch.tensor(
            control_group[["Age", "Exercise", "Medication"]].values,
            dtype=torch.float32,
        ).to(device)
    )
    ...
    experiment_data["ablation_type_1"]["no_cholesterol"] = results
