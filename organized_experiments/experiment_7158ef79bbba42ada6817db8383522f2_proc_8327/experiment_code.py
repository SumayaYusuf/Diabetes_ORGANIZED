...
experiment_data = {"dropout_regularization": {}}
...
# Define the dropout-enhanced feedforward neural network
class PredictorDropout(nn.Module):
    def __init__(self, input_dim):
        super(PredictorDropout, self).__init__()
...
# Hyperparameter tuning with dropout
learning_rates = [0.0005, 0.001, 0.005]
for lr in learning_rates:
    model = PredictorDropout(input_dim=4).to(device)
    ...
    for epoch in range(n_epochs):
        ...
        print(
            f"Dropout, LR {lr}, Epoch {epoch+1}: Train Loss = {train_loss:.4f}, Val Loss = {val_loss:.4f}"
        )
    ...
    print(
        f"Dropout, Learning rate {lr}: Average Treatment Effect on the Treated (ATT): {ATT:.4f}"
    )

    # Save the results for the current learning rate
    experiment_data["dropout_regularization"][f"lr_{lr}"] = results
