...
# Define a simpler feedforward neural network (one hidden layer)
class SimplifiedPredictor(nn.Module):
    def __init__(self, input_dim):
        super(SimplifiedPredictor, self).__init__()
...
for lr in learning_rates:
    model = SimplifiedPredictor(input_dim=4).to(device)
    ...
    experiment_data["remove_hidden_layer"]["default_dataset"] = results
